...
#  P = mel.matlist["ALPHA SCF DENSITY MATRIX"].expand().reshape((nrorb,nrorb)).T #P is in AO basis
  ...
  #Get AO-basis 2-e integrals
  ERI = mel.matlist["REGULAR 2E INTEGRALS"].expand().reshape((nrorb,nrorb,nrorb,nrorb)).T
#
#  if (wfn_type !='rohf'):
...
#    ERI_aa = np.swapaxes(aa2e,1,2)
  ...
  #convert to MO basis, C will do this for us
  ...
  if (wfn_type == "rhf"):
    ERI_aa = twoe_MO_tran(ERI,C_a,C_a)
...
#      bb2e = mel.matlist["BB MO 2E INTEGRALS"].expand().reshape((nrorb,nrorb,nrorb,nrorb)).T
#      ERI_ab = np.swapaxes(ab2e,1,2)
#      ERI_bb = np.swapaxes(bb2e,1,2)
       ERI_aa = twoe_MO_tran(ERI,C_a,C_a)
       ERI_ab = twoe_MO_tran(ERI,C_a,C_b)
       ERI_bb = twoe_MO_tran(ERI,C_b,C_b)
#
    if (wfn_type == 'rohf'):
#      ERI = mel.matlist["REGULAR 2E INTEGRALS"].expand().reshape((nrorb,nrorb,nrorb,nrorb)).T
      P_b = mel.matlist["BETA SCF DENSITY MATRIX"].expand().reshape((nrorb,nrorb)).T #P is in AO basis

      print("Returning ROHF molecular parameters")
      return nrorb, noa, nob, nva, nvb, eref, nrep, C_a, C_b, P, P_b, X, S, H, ERI
